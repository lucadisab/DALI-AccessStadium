...
evi(condizione_tornello):-condizione_tornello(var_X),true==var_X,a(libera_tornello).

a(libera_tornello):-write('Attendo agenti'),nl.

eve(richiesta_ingresso(var_Id,var_Priorita)):-write('Agente arrivato al tornello'),nl,now(var_Timestamp),assert(ordine_agente(var_Id,var_Priorita,var_Timestamp)),a(fai_entrare).

a(fai_entrare):-ordine_agente(var_Id,var_Priorita,var_Timestamp),write('Primo controllo priorita'),nl,var_Priorita==0->ordine_agente(var_Id,var_Priorita,var_Timestamp),not((ordine_agente(var__,var_Priorita,var_Timestamp2),var_Timestamp2<var_Timestamp)),number_chars(var_Id,var_Char),atom_chars(var_IdString,var_Char),atom_concat('agent',var_IdString,var_X),write(var_X),nl,a(message(var_X,send_message(accedi_allo_stadio,var_Me))),format('messaggio mandato priorita 0 a agente ~d',var_Id);nl,ordine_agente(var_Id,var_Priorita,var_Timestamp),not((ordine_agente(var__,var_Priorita,var_Timestamp2),var_Timestamp2<var_Timestamp)),number_chars(var_Id,var_Char),atom_chars(var_IdString,var_Char),atom_concat('agent',var_IdString,var_X),a(message(var_X,send_message(accedi_allo_stadio,var_Me))),write('messaggio mandato priorita 1').

eve(entrato(var_Id,var_Priorita)):-retract(ordine_agente(var_Id,var_Priorita,var__)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_103891,var_Ontology,_103895),_103885),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_103929)),a(message(var_Ag,propose(var_A,[_103929],var_AgI))),retractall(ext_agent(var_Ag,_103967,var_Ontology,_103971)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_103765,var_Ontology,_103769),_103759),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_103835,var_Ontology,_103839)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_103653,var_Ontology,_103657),_103647),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_103709,var_Ontology,_103713)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_103217),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_103251),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_103033).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_102881),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_102829),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_102705,_102707)),clause(agente(_102727,_102729,_102731,var_S),_102723),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_102499,_102501,_102503,var_S),_102495),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_102599,[]),repeat,read(_102599,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_102599).
