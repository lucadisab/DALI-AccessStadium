...
:-assert(biglietto_comprato(false)).

:-assert(informazioni_agente(1,1)).

eve(tornelli_aperti):-a(message(agent2,send_message(sveglia,var_Me))),a(vai_tornello).

a(vai_tornello):-biglietto_comprato(var_X),true==var_X->write('Ho il biglietto'),nl,a(message(agentTornello,send_message(richiesta_ingresso(1,1),var_Me)));write('Non ho il biglietto, lo compro'),nl,a(compra_biglietto).

a(compra_biglietto):-a(message(agentBigliettaio,send_message(richiesta_acquisto(1),var_Me))),write('Sto comprando il biglietto'),nl.

eve(biglietto_acquistato):-write('Ho acquistato il biglietto'),nl,biglietto_comprato(var_X),retract(biglietto_comprato(var_X)),assert(biglietto_comprato(true)),a(vai_tornello).

eve(accedi_allo_stadio):-informazioni_agente(var_Id,var_Priorita),format('Sono agente ~d e sono entrato allo stadio ',var_Id),nl,a(message(agentTornello,send_message(entrato(var_Id,var_Priorita),var_Me))),retract(informazioni_agente),retract(biglietto_comprato).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_91655,var_Ontology,_91659),_91649),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_91693)),a(message(var_Ag,propose(var_A,[_91693],var_AgI))),retractall(ext_agent(var_Ag,_91731,var_Ontology,_91735)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_91529,var_Ontology,_91533),_91523),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_91599,var_Ontology,_91603)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_91417,var_Ontology,_91421),_91411),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_91473,var_Ontology,_91477)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_90981),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_91015),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_90797).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_90645),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_90593),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_90469,_90471)),clause(agente(_90491,_90493,_90495,var_S),_90487),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_90263,_90265,_90267,var_S),_90259),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_90363,[]),repeat,read(_90363,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_90363).
